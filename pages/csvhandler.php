    ...
    if (isset($_FILES["csvFile"]) && $_FILES["csvFile"]["error"] == 0) {
        ...
        if (in_array($fileExtension, $allowedExtensions)) {
            ...
            if (move_uploaded_file($_FILES["csvFile"]["tmp_name"], $ielect)) {
                ...
                // Assuming the CSV format: student_id, email, first_name, last_name, course, department, contact_number
                foreach ($csvData as $row) {
                    ...
                    if (count($row) >= 4) {
                        ...
                        $first_name = $row[2];
                        $last_name = $row[3];
                        $course = isset($row[4]) ? $row[4] : null;
                        $department = isset($row[5]) ? $row[5] : null;
                        $contact_number = isset($row[6]) ? $row[6] : null;
                        
                        // Insert voter data into the database
                        $voterManager = new VoterManager();
                        $voterManager->insertVoter($student_id, $email, $first_name, $last_name, $course, $department, $contact_number);
                    } else {
